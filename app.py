...
# Select page and get its records
if pages:
...
# --- Output Sizes Editor in Sidebar ---
...
if records and image_file and (size_mappings or custom_sizes):
    ...
    for idx, (rec, out_size, is_custom) in enumerate(crops_to_show):
        ...
        base_left, base_top = offs["x"], offs["y"]
        base_w, base_h = offs["w"], offs["h"]
        ...
        if not is_custom:
            ...
            cw, ch = out_size
            new_w = base_w
            new_h = new_w * (cw/ch) ** -1  # base_w / (cw/ch) wrong—should be new_h = new_w / (cw/ch)
            # Actually:
            new_w = base_w
            new_h = new_w / (cw/ch)
            if new_h > base_h:
