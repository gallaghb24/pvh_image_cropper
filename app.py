...
def choose_face_for_window(faces: List[Tuple[int,int,int,int]], l:int, t:int, w:int, h:int):
    """Prefer a face that intersects the window; otherwise pick nearest to window center, tie-break by area."""
    if not faces:
        return None
    window = (l, t, l + w, t + h)
...
def minimal_outside_rect_containing(w, h, ar):
    """Minimal rectangle of aspect 'ar' that contains a w×h rectangle. Returns (W,H)."""
...
def maximal_inside_rect(w, h, ar):
    """Maximal rectangle of aspect 'ar' that fits inside a w×h rectangle. Returns (W,H)."""
    if ar >= w / h:
        W = w
        H = int(round(W / ar))
    else:
...
with zipfile.ZipFile(zip_buf, "w") as zf:
    ...
    # Custom crops (Face-aware toggle respected; WYSIWYG)
    for cw, ch in custom_sizes:
        key_root = f"{cw}x{ch}"
        base_key = f"base_{key_root}"
